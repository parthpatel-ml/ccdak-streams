...
public class AggregationsMain {
    ...
    public static void main(String[] args) {
        ...
        KTable<String, Integer> aggregatedTable = groupedStream.aggregate(
                () -> 0, // initial value of result, here we are calculating length of characters,
                (aggKey, newValue, aggValue) -> aggValue + newValue.length(),
                Materialized.with(Serdes.String(), Serdes.Integer()));
        // now we perform aggregation and output it into output topic.
        // serdes are different than input so need to use Produced.with(Serdes.String(), Serdes.Integer()))
        ...
        KTable<String, String> reducedTable = groupedStream.reduce((aggValue, newValue)
                -> aggValue + " " + newValue + "");
